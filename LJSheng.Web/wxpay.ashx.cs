                ...
                if (return_code == "SUCCESS" && result_code == "SUCCESS")//验证成功
                {
                    ...
                    if (xmldoc.SelectSingleNode("/xml/trade_state").InnerText == "SUCCESS")
                    {
                        ...
                        else
                        {
                            ...
                            {
                                LogManager.WriteLog("微信支付成功", "订单号:" + out_trade_no + ",网银订单号:" + trade_no + ",网上支付金额:" + pay_amount + ",状态:" + result_code + " - " + return_code + "\r\n" + responseContent);
                            }
                            else
                            {
                                LogManager.WriteLog("微信支付成功对账失败", "订单号:" + out_trade_no + ",网银订单号:" + trade_no + ",网上支付金额:" + pay_amount + ",状态:" + result_code + " - " + return_code);
                            }
                            HttpContext.Current.Response.Write(responseContent);
                        }
                    }
