...
namespace LJSheng.Common
{
    public class imgaddimg
    {
        ...
        public static Image Ctjr(Image ewm,string gid)
        {
            //图片里打帐号
            string path = System.Web.HttpContext.Current.Server.MapPath("/uploadfiles/ewmmp/");
            Bitmap img = new Bitmap((path + "ewm.png"));
            try
            {
                ...
                Brush brush = Brushes.Blue; //字体颜色
                g.DrawString(gid, font, brush, 68, 350);
                img.Save(path + gid + ".jpg");
            }
            catch (Exception err)
            {
                LogManager.WriteLog("生成二维码失败", gid + " --- " + err.Message);
            }
            finally
            {
                GC.Collect();
