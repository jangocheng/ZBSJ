...
namespace LJSheng.Common
{
    public class LSession
    {
        ...
        {
            if (!string.IsNullOrEmpty(LJSheng.Common.LCookie.GetCookie("ljsheng")) || HttpContext.Current.Session[SessionName]!= null ||!string.IsNullOrEmpty(HttpContext.Current.Session[SessionName].ToString()))
            {
                JObject paramJson = null;
                if (!string.IsNullOrEmpty(LJSheng.Common.LCookie.GetCookie("ljsheng")))
                {
                    paramJson = JsonConvert.DeserializeObject(LJSheng.Common.DESRSA.DESDeljsheng(LJSheng.Common.LCookie.GetCookie("ljsheng"))) as JObject;
                }
                else
                {
