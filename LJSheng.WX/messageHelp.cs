...
namespace LJSheng.WX
{
    ...
    {
        string url = "http://www.huizhongxin.cc";
        ...
        public string EventHandle(XmlDocument xmldoc)
        {
            ...
            {
                var query = db.wx_keyword.Where(l => l.keyword.Contains("其他")).FirstOrDefault();
                if (query != null)
                {
                ...
                if (Event != null)
                {
                    switch (Event.InnerText)
                    {
                        //事件类型，subscribe(订阅)
                        case "subscribe":
                            query = db.wx_keyword.Where(l => l.keyword.Contains("关注")).FirstOrDefault();
                            if (query != null)
                            {
                                if (query.ltype == 2)
                                {
                        ...
                        case "CLICK":
                            ...
                            {
                                query = db.wx_keyword.Where(l => l.keyword.Contains(EventKey.InnerText)).FirstOrDefault();
                                if (query != null)
                                {
                                    if (query.ltype == 2)
                                    {
        ...
        public string TextHandle(XmlDocument xmldoc)
        {
            ...
            using (EFDB db = new EFDB())
                ...
                if (Content != null)
                {
                    query = db.wx_keyword.Where(l => l.keyword.Contains(Content.InnerText)).FirstOrDefault();
                    if (query != null)
                    {
                        if (query.ltype == 2)
                        {
