...
namespace LJSheng.WX
{
    ...
    public class WXPay
    {
        ...
        /// 2018-08-18 林建生
        /// </remarks>
        public static string Get_RequestHtml(string openid, string order_no, decimal payrmb, string body, string attach)
        {
            ...
            paydic.Add("out_trade_no", order_no);
            ...
            pay += "<out_trade_no><![CDATA[" + order_no + "]]></out_trade_no> ";
            ...
            using (EFDB db = new EFDB())
            {
                ...
                if (b != null)
                {
                    ...
                    if (db.SaveChanges() != 1)
                    {
                        LogManager.WriteLog("支付参数失败", "订单号=" + order_no + "----------参数=" + _req_data);
                    }
                }
                else
                {
                    {
                        LogManager.WriteLog("找不到订单", "订单号=" + order_no + "----------参数=" + _req_data);
                    }
                }
            }
            //通知支付接口，拿到prepay_id
        ...
        public static string Get_Order(string order)
        {
            using (EFDB db = new EFDB())
            {
                ...
                var b = db.lorder.Where(l => l.order_no == order && l.pay_status == 2).FirstOrDefault();
                if (b != null)
                {
